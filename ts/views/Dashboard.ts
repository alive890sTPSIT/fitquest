...
export function dashboard():HTMLElement{
    ...
    dashboard.querySelector("#v-pills-piani-tab")?.addEventListener('show.bs.tab',
        async function (event) {
            dashboard.querySelector("#v-pills-piani")!.innerHTML="";
            dashboard.querySelector("#v-pills-piani")!
            .appendChild(gridPiani((await (PianiController.Read())).data??[]));
        }
    );
    dashboard.querySelector("#v-pills-progressi-tab")?.addEventListener('show.bs.tab',
      async (event) => {
        dashboard.querySelector("#v-pills-progressi")!.innerHTML="";
            dashboard.querySelector("#v-pills-progressi")!
            .appendChild(gridProgressi((await (ProgressiController.Read())).data??[]));
      }
    )
