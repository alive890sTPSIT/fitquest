...
function gridUtenti(utenti: Utente[]): HTMLDivElement {
...
function formNewUtente(): HTMLFormElement {
  ...
  form.onsubmit = async (e) => {
    ...
    const utente: Utente = {
      id: 0,
      nome: (form.querySelector<HTMLInputElement>(`#utente-nome`)!).value,
      username: (form.querySelector<HTMLInputElement>(`#utente-username`)!).value,
      email: (form.querySelector<HTMLInputElement>(`#utente-email`)!).value,
      password_hash: (form.querySelector<HTMLInputElement>(`#utente-password`)!).value,
      data_iscrizione: (new Date((form.querySelector<HTMLInputElement>(`#utente-data-iscrizione`)!.value))).toISOString()
    };
    UtentiController.Create(utente).then((res) => {
      ...
      if (res.success) {
        ...
        // const c = document.createElement("div");
        // c.className = "col-sm-6 col-md-4 mb-3";
        // c.appendChild(cardUtente(formUtente()));
        // document.getElementById("grid-utenti")!.appendChild(c);
      }
    });
  }
...
function cardUtente(form: HTMLFormElement): HTMLDivElement {
  const template = document.createElement("template");
  template.innerHTML = `
<div class="card">
  <div class="card-body">
...
function formUtente(data: Utente): HTMLFormElement {
  const template = document.createElement("template");
  template.innerHTML = `
      <form>
        <div class="mb-3">
 ...
 <button type="button" class="btn btn-danger" data-bs-toggle="modal" data-bs-target="#deleteConfirmModal">
  ...
  (form.querySelector(`#utente-id-${data.id}`) as HTMLElement).textContent = data.id.toString();
  (form.querySelector<HTMLInputElement>(`#utente-nome-${data.id}`)!).value = data.nome;
  (form.querySelector<HTMLInputElement>(`#utente-username-${data.id}`)!).value = data.username;
  (form.querySelector<HTMLInputElement>(`#utente-email-${data.id}`)!).value = data.email;
  (form.querySelector<HTMLInputElement>(`#utente-password-${data.id}`)!).value = data.password_hash;
  (form.querySelector<HTMLInputElement>(`#utente-data-iscrizione-${data.id}`)!).value = new Date(data.data_iscrizione).toISOString().slice(0, 16);
  form.onsubmit = async (e) => {
    e.preventDefault();
    console.log(form.querySelector<HTMLInputElement>(`#utente-data-iscrizione-${data.id}`))
    const utente: Utente = {
      id: data.id,
      nome: (form.querySelector<HTMLInputElement>(`#utente-nome-${data.id}`)!).value,
      username: (form.querySelector<HTMLInputElement>(`#utente-username-${data.id}`)!).value,
      email: (form.querySelector<HTMLInputElement>(`#utente-email-${data.id}`)!).value,
      password_hash: (form.querySelector<HTMLInputElement>(`#utente-password-${data.id}`)!).value,
      data_iscrizione: (new Date((form.querySelector<HTMLInputElement>(`#utente-data-iscrizione-${data.id}`)!.value))).toISOString()
    };
